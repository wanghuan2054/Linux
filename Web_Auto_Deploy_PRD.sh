...
# By Author WangHuan 2020-09-11   MDWETL3
...
for ((i=1; i<=$DEPLOY_SERVER_NUMS; i ++))  
do  
    # tomcat_server 进程名字
    PROCESS_NAME="tomcat_server$i"
	
	# Test Tocmat Server 根目录地址
	TOMCAT_ROOT_DIR="/home/webadm/$PROCESS_NAME"
	...
	then
		...
		echo -e "$TOMCAT_PID Tomcat Server$i Process kill success !"
	fi
	sleep 3
	
	# 将之前版本代码移走备份
	# 先判断War文件是否存在 , 只需要备份三节点的一份代码即可
	if [ $i -eq 3 ]; then
	...
	if [ -n  "$TOMCAT_PID" ]; 
	then
		echo -e "Tomcat_server[$i]  Start Success !"
	else
		echo -e "Tomcat_server[$i]  Start Failed  !"
		exit
	fi

done  

echo -e "\033[36mRealease Complete! \033[0m"
