...
# By Author WangHuan 2020-09-11

#取当前系统时间
time_now=$(date "+%Y%m%d%H%M%S")
# Test Tocmat Server 根目录地址
TOMCAT_ROOT_DIR="/home/webadm/tomcat_server3"
# tomcat_server_tst 进程名字
PROCESS_NAME="tomcat_server3"
# Test Tocmat Server Webapps 目录地址
TOMCAT_WEB_DIR="$TOMCAT_ROOT_DIR/webapps"
# 查找Test Tocmat Server 进程 ID
TOMCAT_PID=`ps -ef | grep -v grep | grep $PROCESS_NAME | awk '{print $2}'`
# 最新源码war地址
LAEST_SOURCECODE_DIR="/home/webadm/Web-Realease"
# Test Tocmat Server war 备份目录
BACKUP_DIR="$LAEST_SOURCECODE_DIR/war_backup"
...
if [ -n  "$TOMCAT_PID" ]; 
then
    echo -e "Tomcat_server[3]  Start Success !"
else
	echo -e "Tomcat_server[3]  Start Failed  !"
	exit
fi

# Tomcat Server 4 发布脚本
# 判断当前Tomcat 进程是否alive，若存活则kill
if [ -n  "$TOMCAT_PID" ]; 
then
	# kill tomcat 进程
...
# 拷贝最新war包到Tomcat Server 4 Webapps
cp $LAEST_SOURCECODE_DIR/$WAR_NAME /home/webadm/tomcat_server4/webapps/$WAR_NAME
if [ $? -eq 0 ]; then
    echo -e "Laest $WAR_NAME Upload Tomcat_server[4] Success !"
else
    echo -e "Laest $WAR_NAME Upload Tomcat_server[4] Failed !"
	# 如果war 包上传失败，退出Shell ， 后续命令不执行
	exit
fi

# 清除Tomcat下的work缓存目录
rm -rf /home/webadm/tomcat_server4/work
if [ $? -eq 0 ]; then
    echo -e "Tomcat_server[4] work Cache Clean Up Success ! "
else
    echo -e "Tomcat_server[4] work Cache Clean Up Failed ! "
fi

# 启动Tomcat server
/home/webadm/tomcat_server4/bin/startup.sh
sleep 5

TOMCAT_PID=`ps -ef | grep -v grep | grep tomcat_server4 | awk '{print $2}'`
echo $TOMCAT_PID
# 判断当前Tomcat 进程是否启动成功
if [ -n  "$TOMCAT_PID" ]; 
then
    echo -e "Tomcat_server[4]  Realease Success !"
else
	echo -e "Tomcat_server[4]  Realease Failed  !"
	exit
fi


echo -e "\033[36m Realease Complete! \033[0m"
